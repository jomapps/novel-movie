...
async function testDurationAdaptiveStructure() {
  ...
  try {
    ...
    console.log(
      `🎭 Format: ${typeof project.movieFormat === 'object' ? project.movieFormat.name : project.movieFormat}`,
    )
    console.log(
      `🎨 Style: ${typeof project.movieStyle === 'object' ? project.movieStyle.name : project.movieStyle}`,
    )
    ...
    if (!structureResponse.ok) {
      ...
      throw new Error(
        `Structure generation failed: ${structureResponse.status} - ${errorData.error}`,
      )
    }
    ...
    if (structure.durationCompliance) {
      ...
      console.log(
        `Story Beats: ${compliance.actualMetrics.storyBeatsCount}/${compliance.constraints.maxStoryBeats}`,
      )
      console.log(
        `Characters: ${compliance.actualMetrics.characterCount}/${compliance.constraints.maxCharacters}`,
      )
      console.log(
        `Subplots: ${compliance.actualMetrics.subplotCount}/${compliance.constraints.maxSubplots}`,
      )
      console.log(
        `Pacing: ${compliance.actualMetrics.averageBeatsPerMinute.toFixed(2)}/${compliance.constraints.beatsPerMinute} beats/min`,
      )
    ...
    if (structure.adaptiveStructure) {
      ...
      if (adaptive.acts && adaptive.acts.length > 0) {
        ...
        adaptive.acts.forEach((act, i) => {
          console.log(
            `Act ${act.actNumber}: ${act.name} (${act.duration} min) - ${act.description?.substring(0, 100)}...`,
          )
        })
      }
    ...
    console.log(
      `Story Beats: ${storyBeats} ${storyBeats <= 4 ? '✅' : '❌'} (should be ≤4 for 1-minute)`,
    )
    console.log(
      `Character Arcs: ${characterArcs} ${characterArcs <= 2 ? '✅' : '❌'} (should be ≤2 for 1-minute)`,
    )
