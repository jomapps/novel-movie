...
async function testBasicStoryGeneration() {
  ...
  try {
    ...
    console.log(
      '  - Genres:',
      project.primaryGenres?.map((g) => g.name || g).join(', ') || 'Not set',
    )
    console.log('  - Premise:', project.corePremise ? 'Present' : 'Not set')
    console.log(
      '  - Audience:',
      project.targetAudience?.map((a) => a.name || a).join(', ') || 'Not set',
    )
    ...
    const checks = {
      ...
      'Has project reference':
        content.includes(project.name) || content.includes(project.projectTitle || ''),
      'Has structured format':
        content.includes('Act I') || content.includes('**') || content.includes('STORY'),
      'References project format': content.includes(
        project.movieFormat?.name || project.movieFormat || '',
      ),
      'References project style': content.includes(
        project.movieStyle?.name || project.movieStyle || '',
      ),
    }
    ...
    if (
      ...
      error.message.includes('replace-with-actual-project-id')
    ) {
      console.log('\n💡 To run this test:')
