...
export default function CreateProjectPage() {
  ...
  const [moodOptions, setMoodOptions] = useState<MoodDescriptor[]>([])
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [formatsData, stylesData, seriesData, genresData, audienceData, toneData, moodData] =
          await Promise.all([
            formatsRes.json(),
            stylesRes.json(),
            seriesRes.json(),
        ...
        if (moodData.success) setMoodOptions(moodData.data)
      } catch (error) {
  ...
  const handleAIAutoFill = async () => {
    ...
    try {
      const requestData = {
        ...
        mood: formData.mood.length > 0 ? formData.mood : undefined,
      }
  ...
  const moodOptionsForSelect = moodOptions.map((mood) => ({
    value: mood.id,
    label: mood.name,
  }))
          ...
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 shadow-sm rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                <MultiSelect
                  value={formData.mood}
                  onChange={(values) => handleInputChange('mood', values.slice(0, 2))}
                  options={moodOptionsForSelect}
